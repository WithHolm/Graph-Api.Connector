Function Invoke-GraphCall
{
    ...
    process
    {
        ...
        if ($return -ne $null)
        {       
            if((Get-GraphAPIConfigFile).odata.UseClasses)
            {
                $odata = Get-CallOdata -Inndata $return    
                
                if ($odata.Entity)
                {
                    $return.psobject.properties.remove("@odata.context")
                    $process = @($return)
                }
                else 
                {
                ...
                $CA = (get-GraphobjectNames -Version $odata.version).foreach{"$($_.name)_$($_.version)"}
                $odataInfo = (Get-AllExpandTypes).where{($_.odataname -eq $odata.OdataTag) -and ($_.version -eq $version)}
                
                $return = $process.foreach{
                    Write-verbose "Processing object $($odata.Entityname): $($_.id)"
                    Convertto-GraphClass -data $_ -classname "$($odataInfo.Entityname)_$version" -classarray $CA
                }
